...
- 校验：帧头到数据末位逐字节累加取低8位
- **length 字段只算 payload，不含 type**（主机 `dataAgreeAnalys`: `data[3]=length`, `data[4]=type`, `memcpy(data+5, data[3])`）
- **握手协议（参考 DurUI + 用户确认，最终语义）**：
  - 主机超时检测的是"**有没有收到数据帧**"，不是握手应答。摄像头持续发数据帧 → 主机不重发握手；仅进程重启/断连间隙主机才重发握手请求。
  - 握手请求帧（主机发，完整字节）：`5A 97 98 0B 09 "Please Link" A5 A5`（帧尾两个 A5）。摄像头按 0x5A 对齐匹配**完整帧**，不用子串。
  - 握手应答帧（摄像头发）：`5A A7 97 0F 09 "Play Aplication" <chk> A5`。payload `"Play Aplication"`（15字符，文档拼写缺p，主机硬匹配）。
  - 摄像头收到完整握手请求 → 应答 + 记时间戳。**入口不短路**（`_connected` 仅状态标记，主机任何时候重发都要能重新应答）。
  - **100ms 起步延迟**：握手应答后等 100ms 才开始发数据帧（静默期内跳过 `send_id_data`），之后每帧发。未握手前不发数据。

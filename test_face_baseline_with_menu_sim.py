...
def main():
    ...
    lvgl_init()
    import gc as _gc
    print("[baseline] init done, mem_free=%d" % _gc.mem_free())

    # ── 模拟 CamerAi main.py 主菜单期：占内存 + 建 LVGL 对象 + 跑几秒 task_handler ──
    # 复现"开机 init 后隔了主菜单再跑 AI"的环境，验证是否主菜单污染致卡。
    print("[baseline] simulating main menu phase...")
    # 占 ~850KB 内存（模拟图标/字体预读）
    print("[baseline]   pre-junk alloc")
    _junk = [bytearray(100*1024) for _ in range(8)]  # ~800KB
    print("[baseline]   post-junk alloc, mem_free=%d" % _gc.mem_free())
    # 建一批 LVGL 对象（模拟主菜单卡片）
    print("[baseline]   pre-lvgl-objs")
    _menu_objs = []
    scr = lv.scr_act()
    for i in range(9):
        o = lv.obj(scr)
        o.set_size(80, 80)
        o.set_pos(i*10, i*10)
        o.add_flag(lv.obj.FLAG.HIDDEN)  # ← 关键：建后立刻隐藏，不 flush 显示
        _menu_objs.append(o)
    print("[baseline]   post-lvgl-objs(all hidden), mem_free=%d" % _gc.mem_free())
    _gc.collect()
    print("[baseline] menu sim done (skip task_handler loop), mem_free=%d, starting face_det thread" % _gc.mem_free())
